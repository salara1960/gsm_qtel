...
char stx[256];
    ...
    ix <<= 8;
    ix |= (CS_ROM & 0x00ff);
    if (subcard[np].type) ix <<= 4;//for PCIe card
...
unsigned short adi;
	...
	b_s = ioread16(adra);
#endif
	if (!(b_s & MASK_PIN)) { loop = 0; ok = 1; break; } else sho--;
	if (!sho) loop = 0;
    }
...
if (node == Major) {//Altera
	...
	case 6:// чтение всего буфера, конец пакета - только 0x0d
		...
		if (adra) {
			...
			if (bbs == 0x0d) mores = 0;
			    ...
			    bbb = (ioread8(adra+ix)) & RDEMP;
			    if ((!bbb) && (mycount > 0) && (ret < 512)) mores = 1;
							           else mores = 0;
			}
		    }
		    if (ret > 0) flg = 1;
	...
	case 0x0C:// чтение всего буфера, почти как case 8, только конец пакета - 0x0a
		...
		while (mores) {
			...
			bbb = (ioread8(adra + ix)) & RDEMP;
			if ((!bbb) && (mycount > 0) && (ret < 512)) mores = 1; else mores = 0;
		    }
		}
		if (ret > 0) flg = 1;
	    }
	...
	break;
	case 0x0A://чтение заданного количества байт из буфера ат_команд задается значением переменной RK_len_buff[rk],но не более 512
	    ret = 0;
	    rk = offsets;
	    if (rk < all_radio) {
	...
	break;
	case 0x0B://чтение заданного количества байт из буфера ат_команд задается значением переменной RK_len_buff[rk], но не более 2048
	    ret = 0;
	    rk = offsets;
	    if (rk < all_radio) {
		mycount = RK_len_buff[rk];
...
*/
	...
	case 15 ://case 11 ://"set_param_rtp"  +  case 8 ://"set_signaling_RTP"
		...
		if (init_all_mix_param_rtp(np, rk, bt)) ret = 10;
	    }
	break;
...
unsigned char bt = 0x11, bbb = 0, sbrd = 0x10;
unsigned short word;
...
err_out:
    ...
    for (k = 0; k < max_brd; k++) {
	if (BaseAdr[k]) iounmap(BaseAdr[k]);
	if (gsm_board[k]) {
