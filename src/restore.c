...
struct sigaction Act_r, OldAct_r;
struct sigaction Act_t, OldAct_t;
struct sigaction Act_i, OldAct_i;
struct sigaction Act_s, OldAct_s;
...
//------------  set signal route functions ------------------
    ...
    SIGSEGVs=1;  Act_s.sa_handler = &_SegvSig;  Act_s.sa_flags = 0;  sigaction(SIGSEGV, &Act_s, &OldAct_s);
...
//------------------------------ читаем bin-файл ----------------------------------------
    ...
    if (i==-1) {
	memset(chaka,0,256);
	sprintf(chaka,"%s. Can't read file %s\n",CurTime(),fname);
	printf(chaka);
	if (bin != NULL) free(bin);
        return -1;
    } else {
	memset(chaka,0,256);
	sprintf(chaka,"%s. File %s readed Ok (size: file=%d buf=%d, pk_all=%d, last_pk_len=%d)\n",
...
//---------------- открываем device радиоканалов ----------------------------------------
	...
	memset(chaka,0,256);
	sprintf(chaka,"Can't open /dev/k16\n");
	printf(chaka);
	if (bin != NULL) free(bin);
        return -1;
    }
...
//----------------------------  open TMR device ----------------------------------------------
	...
	memset(chaka,0,256);
	sprintf(chaka,"Can't open /dev/tmr (%s)\n",strerror(errno));
	printf(chaka);
	if (bin != NULL) free(bin);
        return -1;
    }
...
//**************************************************************************************************
	...
	switch (faza) {
		...
		tp=0;
		if (strstr(module,"M10ER01A")!=NULL) tp=10;
		else if (strstr(module,"M12ER01A")!=NULL) tp=12;
		if (tp) {
...
*/
	...
	switch (rx_faza) {
	    ...
	    break;
	    case 1:
		if ((RXdone(Def_RK)) && (COM.rx_numb<max_buf_len) && (COM.rx_ready==0)) {
		    memset(rxd,0,max_buf_len);
		...
		if (COM.rx_ready==1) rx_faza = 2;
		    ...
		    if (check_delay(tmr_rx)) { if (COM.rx_numb==0) rx_faza = 0; else rx_faza = 2; }
		}
	    break;
	    case 2:
		if (strstr(pointerRX,"OK")!=NULL) COM.StatusAdp |= ReciveOK;
		else
		if (strstr(pointerRX,"ERROR")!=NULL) COM.StatusAdp |= ReciveERR;
		else
		if (strstr(pointerRX,"+CME")!=NULL) COM.StatusAdp |= ReciveCME;
		else {
		    COM.StatusAdp |= ReciveCom;// какая-то другая команда (точнее ответ на команду)
		    if (strstr(pointerRX,"Revision: ")!=NULL) {
			if (!pgm) {
...
//**************************************************************************************************
    ...
    memset(chaka,0,512);
    sprintf(chaka,"%s. time=%d sec., chan=%d, err=%d/%d, packs=%d, revision: %s | %s\n",
